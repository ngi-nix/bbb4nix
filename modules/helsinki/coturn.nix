...
in
{
  options = {
    services.coturn = {
      ...
      staticAuthSecretFile = mkOption {
        type = types.str;
        default = "";
        description = "Path to read static auth secret from and append to config.";
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.coturn = {
      ...
      preStart = ''
        set -e
        cat ${configFile} > /run/turnserver/conf
        echo "static-auth-secret=$(cat ${cfg.staticAuthSecretFile})" >> /run/turnserver/conf
      '';

      serviceConfig = {
        Type = "simple";
        ExecStart = "${pkgs.coturn}/bin/turnserver -c /run/turnserver/conf ${cfg.extraParams}";
