...
# It was modified to fit our needs, so it has:
# - One shared repository instead of 3
# - extraDeps support
# - Hide Gradle warnings (why show them in the first place?)
...
let
  ...
  mkRepo = project: type: deps: buildEnv {
    ...
    paths = map mkDep deps ++ mkModuleMetadata deps ++ mkSnapshotMetadata deps ++ extraDeps;
  };
